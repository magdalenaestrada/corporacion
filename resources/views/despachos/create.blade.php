            ...
            <div class="card-body">
                ...
                <script>
                    $(document).ready(function() {
                        ...
                        // =========================================================
                        // ✅ CAMBIO 1: toNum() y formatNumber()
                        // - Mostrar y guardar SIEMPRE sin miles y con 3 decimales
                        // - Mostrar 1100.800 en lugar de 1.100.800
                        // =========================================================
                        ...
                        function toNum(v){
                            ...
                            // Si tiene . y , => el último es decimal
                            if (lastDot !== -1 && lastCom !== -1) {
                                const decPos = Math.max(lastDot, lastCom);
                                const decimalsRaw = s.slice(decPos + 1).replace(/[^\d]/g, '');
                                let all = s.replace(/[.,]/g, '');

                                const integers = all.slice(0, all.length - decimalsRaw.length);
                                s = integers + '.' + decimalsRaw;
                                return parseFloat(s);
                            }
                            ...
                            // Solo punto => punto decimal (NO miles)
                            s = s.replace(/,/g, '');
                            return parseFloat(s);
                        }
                        ...
                        function actualizarTotalTMH() {
                            ...
                            // ✅ CAMBIO 2: mostrar total sin miles y con 3 decimales
                        ...
                        function formatMasomenos(n){
                            ...
                            // Convertir de KG a TMH (según tu comentario)
                            const tmh = x / 1000;
                        ...
                        $btnAdd.on('click', function(){
                            ...
                            // ✅ CAMBIO 3: al guardar, normalizar bruto/tara/neto a formato 3 decimales sin miles
                            const bruto = formatNumber(toNum($('#bruto').val()));
                            const tara  = formatNumber(toNum($('#tara').val()));
                            const neto  = formatNumber(toNum($('#neto').val()));
                ...
                <form action="{{ route('despachos.store') }}" method="POST" id="despachoForm">
                    ...
                    <div class="mb-3">
                        ...
                        {{-- ✅ CAMBIO 4: se muestra 1100.800 (sin miles) porque el JS ya lo setea con toFixed(3) --}}
