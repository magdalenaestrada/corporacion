...
<div class="container-fluid">
    ...
    <div class="card-body">
      ...
      <div class="table-responsive">
        ...
        <style>
          ...
          .switch-centered{
            ...
            gap:.5rem; padding-left:0;
          }
          .switch-centered .form-check-input{ float:none; margin-left:0; }
          .switch-centered .form-check-label{ margin:0; }
        </style>
      </div>
            ...
            <div class="form-group row">
              <div class="form-group col-md-4 g-3">
                ...
                <div class="input-group">
                  <input type="text" name="ruc_empresa" id="ruc_empresa" class="form-control"
      ...
      <script>
        $(document).ready(function() {
          ...
          // =============== HELPERS (FIX) =================
          // - toNum: convierte cualquier formato a number (soporta 52.960, 1.845.520, 2.920,000, etc.)
          // - formatNumber: deja tu UI con separadores (NO tocamos el resto)
          // - fixed3: SOLO para totalTMH (sin miles)
          function toNum(v){
            ...
            if (dots === 1 && /\.\d{3}$/.test(s)) return parseFloat(s);

            if (dots > 1) {
              const last = s.lastIndexOf('.');
              const dec = s.slice(last + 1).replace(/[^\d]/g,'');
              const ints = s.slice(0, last).replace(/\./g,'').replace(/[^\d-]/g,'');
              return parseFloat(ints + '.' + dec);
            }
          ...
          function formatNumber(n){
            ...
            return x.toLocaleString('es-CL'); // tu formato visual
          }

          function fixed3(n){
            const x = Number(n);
            if (Number.isNaN(x)) return '';
            return x.toFixed(3); // 1845.520
          }
          ...
          // Auto-llenado (NO tocamos tu formato visual)
  ...
  $('input[name="masomenos"]').val(diferencia.toFixed(3));
          ...
          // Agregar/Actualizar (NO tocamos formatos de bruto/tara/neto, solo se guardan como están)
          $btnAdd.on('click', function(){
            ...
            if(!editingUid && usados.has(NroSalida)) return;
            ...
            $('#formRetiro')[0].reset();
            if(keepManual) desactivarEdicion(true);
            else desactivarEdicion(false);
          });
      ...
      <form action="{{ route('despachos.store') }}" method="POST" id="despachoForm">
        ...
        <div class="mb-3">
          <label for="totalTMH" class="form-label">
            <strong>{{ __('TONELAJE TOTAL DESPACHADO TMH') }}</strong>
          </label>
