            ...
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <!-- Modal Ranking 👑 -->
                        <div class="modal fade" id="modalRanking" tabindex="-1" aria-labelledby="rankingLabel"
                            aria-hidden="true">
                                        ...
                                        <div class="d-flex justify-content-around align-items-end gap-3 flex-wrap"
                                            ...
                                            @foreach ($conteoCierres->sortByDesc('count')->take(5) as $user)
                                                ...
                                                <div class="bar-wrapper rank-{{ $rank }}">
                                                    ...
                                                    {{-- nombre / cierres --}}
                                                    <div class="meta">
                                                        <div class="name" title="{{ $user['name'] }}">{{ $user['name'] }}
                                                        </div>
                        ...
                        <style>
                            ...
                            .rank-1 .bar {
                                background: linear-gradient(180deg, #ffec54 0%, #d4a400 100%);
                            }

                            .rank-2 .bar {
                                background: linear-gradient(180deg, #d9d9d9 0%, #9e9e9e 100%);
                            }

                            .rank-3 .bar {
                                background: linear-gradient(180deg, #d9995f 0%, #8b5a2b 100%);
                            }
                            ...
                            <div class="row g-2 align-items-center">
                                <div class="col-md-5">
                                    <input type="text" name="search" value="{{ request('search') }}"
                                        class="form-control" placeholder="Buscar por muestra, cliente, peso o lote">
                                </div>

                                <div class="col-md-2">
                                    <select name="estado" class="form-select">
                                        <option value="">🔁 Todos los estados</option>
                                        <option value="CIERRE" {{ request('estado') == 'CIERRE' ? 'selected' : '' }}>✅
                                            Cierre</option>
                                        <option value="PROVISIONAL"
                                            {{ request('estado') == 'PROVISIONAL' ? 'selected' : '' }}>🟡 Provisional
                                        </option>
                                        <option value="SIN CIERRE"
                                            {{ request('estado') == 'SIN CIERRE' ? 'selected' : '' }}>
                                            🚫 Sin estado asignado
                                        </option>
                                    </select>
                                </div>
                                ...
                                <div class="col-md-2">
                                    <div class="d-flex gap-2">
                                        ...
                                        <button type="button" class="btn btn-info flex-fill" data-bs-toggle="modal"
                            ...
                            @if (count($liquidaciones) > 0)
                                ...
                                <tbody>
                                    @foreach ($liquidaciones as $liquidacion)
                                        <tr class="@if (in_array(strtoupper($liquidacion->cliente->datos_cliente), [
                                                'BLENDING',
                                                'CHAVEZ CORNEJO JURGEN LUIS',
                                                'INNOVA CORPORATIVO S.A.',
                                            ])) fila-cliente-oculta @endif">
                                            ...
                                            <td>
                                                <span
                                                    class="badge 
                                        {{ $liquidacion->estado == 'CIERRE'
                                            ? 'bg-success'
                                            : ($liquidacion->estado == 'PROVISIONAL'
                                            ...
                                            <td>{{ $liquidacion->creator->name ?? 'N/A' }}
                                                ({{ $liquidacion->created_at->format('d/m/Y - H:i:s') }})
                                            </td>
                                            <td>{{ $liquidacion->lastEditor->name ?? 'N/A' }}
                                                ({{ $liquidacion->updated_at->format('d/m/Y - H:i:s') }})</td>


                                            <td>
                                                <a href="{{ route('liquidaciones.show', $liquidacion->id) }}"
                                                    class="btn btn-secondary btn-sm">{{ __('VER') }}</a>

                                                @if ($liquidacion->estado !== 'CIERRE' && $liquidacion->estado !== 'PROVISIONAL')
                                                    <form action="{{ route('duplicate', $liquidacion->id) }}"
                                                        method="POST" style="display:inline;">
                                                        @csrf
                                                        <button type="submit"
                                                            class="btn btn-success btn-sm">{{ __('CIERRE') }}</button>
                                                    </form>
                                                @endif
                                                @can('update', $liquidacion)
                                                    <a href="{{ route('liquidaciones.edit', parameters: $liquidacion->id) }}"
                                                        class="btn btn-warning btn-sm">{{ __('EDITAR') }}</a>
                                                @endcan
                                                <a href="{{ route('liquidaciones.print', $liquidacion->id) }}"
                                                    class="btn btn-primary btn-sm">{{ __('IMPRIMIR') }}</a>
                                                <a href="{{ route('liquidaciones.print-acta', $liquidacion->id) }}"
                                                    class="btn btn-primary btn-sm">{{ __('ACTA') }}</a>
                                                <form action="{{ route('liquidaciones.destroy', $liquidacion->id) }}"
                                                    method="POST" style="display:inline;" class="form-eliminar">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                            ...
                            @else
                                <tr>
                                    <td colspan="10" class="text-center text-muted">
                                        {{ __('No hay datos disponibles') }}</td>
                                </tr>
                            @endif
                        </table>
    ...
    @push('js')
        ...
        <style>
            ...
            thead th {
                ...
                z-index: 1;
                background: #fff;
                /* o #f8f9fa si usas la clase table-light */
            }
        </style>
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                document.querySelectorAll('form[id^="duplicate-form-"]').forEach(form => {
                    form.addEventListener('submit', function(event) {
                        ...
                        xhr.setRequestHeader('X-CSRF-TOKEN', this.querySelector('input[name="_token"]')
                            .value);
                        ...
                        xhr.onload = function() {
                            if (xhr.status >= 200 && xhr.status < 300) {
                                ...
                                alert(response.success ? 'Registro duplicado exitosamente!' :
